...
public class Maze_Skeleton : Actor {
	...
	void Update(){
		//animation
		if (moveVector.magnitude < moveDeadZone || moveVector.magnitude > visionDistance){
		...
		if (!animator.GetBool("Attacking") && moveVector.magnitude < visionDistance){
	...
	IEnumerator AI_Tick(){
		while (true){
			if (this.IsActive()){
				...
				//do nothing if player is too far 
